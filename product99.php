...
function printTable(tableId) {
    ...
    var imgTags = document.querySelectorAll('.report-image');

    if (imgTags.length > 0) {
        var imgContents = '';
        imgTags.forEach(function(imgTag) {
            imgContents += '<center>' + imgTag.outerHTML + '</center>';
        });
        printContents = imgContents + printContents;
    }

    var originalContents = document.body.innerHTML;
    document.body.innerHTML = printContents;

    // Alternatively, you can use a separate print stylesheet for better control
    var style = document.createElement('style');
    style.innerHTML = '@media print { .report-image { display: block; position: fixed; top: 0; left: 0; right: 0; margin: auto; max-width: 100%; height: auto; } }';
